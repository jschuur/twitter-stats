...
export async function readAccounts({ username, clean } = {}) {
  const { db } = await connectToDatabase();

  const filter = username ? { username } : {};
  const projection = clean ? cleanProjection(privateFieldsAccounts) : {};
...
export async function updateAccountData(username, data) {
  const { db } = await connectToDatabase();

  try {
    await db
      .collection('accounts')
      .updateOne({ username }, { $set: { last_update: new Date().toISOString(), ...data } });
  } catch ({ message }) {
...
export async function writeSnapshot(snapshot) {
  const { db } = await connectToDatabase();
  const { username, date } = snapshot;

  // Add date if needed
  if (!date) snapshot.date = dateFormat(new Date(), 'yyyy-MM-dd');
