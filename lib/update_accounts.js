...
import { readAccounts, writeSnapshot, writeAccountData, writeTweets } from '../lib/db';
...
async function updateTweets(account) {
  ...
  if (tweets.length) {
    logger.info(`Retreived ${pluralize('tweet', tweets.length, true)} for ${username}`);
...
async function updateAccount({ account, skipTweets }) {
  ...
  await writeAccountData(username, { profile });
...
export default async function updateAccounts({ skipTweets }) {
  let results = {};
  const accounts = await readAccounts({ fullDetails: true });
